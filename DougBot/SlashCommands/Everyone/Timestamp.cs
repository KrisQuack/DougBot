...
public class Timestamp : InteractionModuleBase
{
    ...
    public async Task Task(
        [Summary(description: "The date to convert to a timestamp. Format: 04/Jul/2000")]
        [Autocomplete(typeof(DateAutocompleteHandler))]
        string date,
        [Summary(description: "The time to convert to a timestamp. Format: 12:00")]
        [Autocomplete(typeof(TimeAutocompleteHandler))]
        string time,
        [Summary(description: "The time zone to use. Format: America/Los_Angeles")]
        ...
        string timezone)
    {
        ...
        if (!TZConvert.KnownIanaTimeZoneNames.Any(x => x == timezone))
            throw new Exception(
                "Invalid timezone provided, please search for it in the autofill if you are unsure *(e.g. America/Los_Angeles)*");
        // Get the time zone
...
public class TimezoneAutocompleteHandler : AutocompleteHandler
{
        ...
        IAutocompleteInteraction autocompleteInteraction, IParameterInfo parameter, IServiceProvider services)
    {
        ...
        {
            var results = timezones.Where(x => x.ToLower().Contains(current.ToLower()))
