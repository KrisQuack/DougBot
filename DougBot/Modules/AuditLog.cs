...
{
    public class AuditLog_UserJoined : INotificationHandler<UserJoinedNotification>
    {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    var embed = new EmbedBuilder()
                        ...
                        .WithAuthor($"{notification.User.Username} ({notification.User.Id})", notification.User.GetAvatarUrl())
                        .AddField("Account Age", (DateTime.UtcNow - notification.User.CreatedAt.UtcDateTime).TotalDays.ToString("0.00") + " days")
                        .WithTimestamp(DateTime.UtcNow)
                        .Build();
                    var settings = await new Mongo().GetBotSettings();
                    await notification.User.Guild.GetTextChannel(Convert.ToUInt64(settings["log_channel_id"])).SendMessageAsync(embed: embed);
                    // Save to database
                    ...
                    if (member == null)
                    {
                        ...
                        var newMember = new BsonDocument
                        {
                            ...
                            { "joined_at", GuildMember.JoinedAt.HasValue ? GuildMember.JoinedAt.Value.UtcDateTime : BsonNull.Value },
    ...
    public class AuditLog_UserLeft : INotificationHandler<UserLeftNotification>
    {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    var embed = new EmbedBuilder()
                        ...
                        .Build();
                    var settings = await new Mongo().GetBotSettings();
                    await notification.Guild.GetTextChannel(Convert.ToUInt64(settings["log_channel_id"])).SendMessageAsync(embed: embed);
                    // Update the database
                    var member = await new Mongo().GetMember(notification.User.Id);
                    if (member != null)
                    {
    ...
    public class AuditLog_GuildMemberUpdated : INotificationHandler<GuildMemberUpdatedNotification>
    {
...
{
                        embed.AddField("Nickname", $"{userDict["nick"]} -> {after.Nickname}", inline: false);
                    ...
                    {
                        embed.AddField("Name", $"{userDict["name"]} -> {after.Username}", inline: false);
                    ...
                    {
                        embed.AddField("Global Name", $"{userDict["global_name"]} -> {after.GlobalName}", inline: false);
                    ...
                    {
                        var settings = await new Mongo().GetBotSettings();
                        await notification.NewUser.Guild.GetTextChannel(Convert.ToUInt64(settings["log_channel_id"])).SendMessageAsync(embed: embed.Build());
                    }
                    // Update the database
                    if (updateLog["changes"].AsBsonDocument.ElementCount > 0)
                    {
                        // Append the update log to the member's edits array
                        if (!userDict.Contains("edits"))
                        {
                            userDict["edits"] = new BsonArray();
                        }
    ...
    public class AuditLog_MessageReceived : INotificationHandler<MessageReceivedNotification>
    {
    ...
    public class AuditLog_MessageDeleted : INotificationHandler<MessageDeletedNotification>
    {
        public async Task Handle(MessageDeletedNotification notification, CancellationToken cancellationToken)
        {
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    var settings = await new Mongo().GetBotSettings();
                    await channel.Guild.GetTextChannel(Convert.ToUInt64(settings["log_channel_id"])).SendMessageAsync(embeds: embeds.ToArray());
                    // Update the database
    ...
    public class AuditLog_MessageUpdated : INotificationHandler<MessageUpdatedNotification>
    {
            ...
            _ = Task.Run(async () =>
            {
                    ...
                    var updateLog = new BsonDocument
                {
                    ...
                    if (embed.Fields.Any())
                    {
                        ...
                        var settings = await new Mongo().GetBotSettings();
                        await channel.Guild.GetTextChannel(Convert.ToUInt64(settings["log_channel_id"])).SendMessageAsync(embed: embed.Build());
                    }
                    // Update the database
                    if (updateLog["changes"].AsBsonDocument.ElementCount > 0)
                    {

                        if (!dbMessage.Contains("edits"))
                        {
                            dbMessage["edits"] = new BsonArray();
                        }
    ...
    //////////////////////////////////////////////////
    public class AuditLog_ReadyHandler : INotificationHandler<ReadyNotification>
    {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        foreach (var channel in activeChannels)
                        {
                            ...
                            var dbMessages = await mongo.GetMessagesByQuery(channel.Id.ToString(), after: cutoff);
                            ...
                            foreach (var message in messages)
                            {
                                try
                                {
                                    if (!message.Author.IsBot)
                                    {
                                        ...
                                        var dbMessage = dbMessages.FirstOrDefault(x => x["_id"].AsString == message.Id.ToString());
                                        var newMessage = new BsonDocument
                                        {
                                            ...
                                            { "attachments", new BsonArray(message.Attachments.Select(attachment => attachment.Url)) },
                            ...
                            try
                            {
                                ...
                                if (dbMember == null)
                                {
                                    var newMember = new BsonDocument
                                    {
                                        ...
                                        { "joined_at", member.JoinedAt.HasValue ? member.JoinedAt.Value.UtcDateTime : BsonNull.Value },
                                ...
                                else
                                {
                                    ...
                                    if ((dbMember["global_name"] == BsonNull.Value ? null : dbMember["global_name"]) != member.GlobalName)
                                    {
                                        dbMember["global_name"] = member.GlobalName != null ? member.GlobalName : BsonNull.Value;
                                        hasChanged = true;
                                    }
