        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var logMessage = notification.LogEvent.Properties.TryGetValue("Message", out var messageContext)
                    ? messageContext.ToString().Length > 4000
                        ? messageContext.ToString()[..4000]
                ...
                var logChannel = _client.Guilds.FirstOrDefault()
                    ?.GetTextChannel(Convert.ToUInt64(settings.LogChannelId));
