    ...
    public class Verification : InteractionModuleBase
    {
        ...
        public async Task task()
        {
            ...
            var button = new ButtonBuilder()
                ...
                .WithCustomId("verify");
            // Send the message
            await Context.Channel.SendMessageAsync(embed: embed, components: new ComponentBuilder().WithButton(button).Build());
        ...
        public async Task verify()
        {
            ...
            {
                // Get all jpegs
                var jpeg_files = Directory.GetFiles("Data", "*.jpg");
                // Select a random jpeg
                var random = new Random();
                var jpeg = jpeg_files[random.Next(jpeg_files.Length)];
                ...
                if (user == null)
                {
                   await RespondAsync($"An error occurred. Please try again later or message {Context.Client.CurrentUser.Mention}", ephemeral: true);
                    return;
                }
                ...
                for (int i = 1; i <= 10; i++)
                {
                    buttons.Add(new ButtonBuilder().WithLabel(i.ToString()).WithStyle(ButtonStyle.Primary).WithCustomId($"verify_answer_{i}"));
                }
                ...
                await RespondWithFileAsync(jpeg, $"{Context.User.Id}.jpg", "Count the number of bell peppers in the image below!", ephemeral: true, components: component.Build());
            }
            catch (Exception e)
            {
        ...
        public async Task verify_answer(string option)
        {
            ...
            if (user["verification"].AsString == number.ToString())
            {
                ...
                var guild_user = (SocketGuildUser)Context.User;
                await guild_user.AddRoleAsync(role);
                // If the user account is created less than one week, time out for the remainder of the week
                if (DateTime.Now - guild_user.CreatedAt < TimeSpan.FromDays(7))
                {
                    var remaining = TimeSpan.FromDays(7) - (DateTime.Now - guild_user.CreatedAt);
                    await FollowupAsync("You've been verified! Welcome to The Doug District! You're account is less than a week old, so you'll be able to chat in a week. In the meantime, feel free to explore the channels and pick your roles!", ephemeral: true);
                    await guild_user.SetTimeOutAsync(remaining);
                }
                else
                {
                    await FollowupAsync("You've been verified! Welcome to The Doug District! Feel free to explore the channels and pick your roles!", ephemeral: true);
                }
            }
            else
            {
